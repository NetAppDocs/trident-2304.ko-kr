...
일반적으로 Astra Trident를 처음 설치할 때 사용했던 방법과 동일한 방법을 사용하여 업그레이드해야 합니다. Trident 운영자를 사용하여 업그레이드를 시도하기 전에 link:upgrade-trident.html#upgrade-trident.html#step-3-select-an-upgrade-method["업그레이드 방법을 선택합니다"]을 검토하십시오.
...
NOTE: 상태가  `Updating`로 표시되면 진행하기 전에 문제를 해결해야 합니다. 가능한 상태 값 목록은 link:../trident-get-started/kubernetes-deploy-operator.html["여기"^]을 참조하십시오.
...
. (선택 사항) 설치 매개변수를 수정해야 하는 경우  `TridentProvisioner` spec을 업데이트하십시오. 여기에는  `tridentImage`,  `autosupportImage` 값 변경, 개인 이미지 저장소 변경, 네임스페이스 범위 연산자를 삭제한 후 클러스터 범위 연산자를 설치하기 전에  `imagePullSecrets`) 제공 등의 변경 사항이 포함될 수 있습니다. 업데이트 가능한 매개변수의 전체 목록은 link:../trident-get-started/kubernetes-customize-deploy.html#configuration-options["구성 옵션"]을 참조하십시오.
+
[listing]
----
