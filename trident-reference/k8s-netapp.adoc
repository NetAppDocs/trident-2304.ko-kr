...
https://www.netapp.com/google-cloud/netapp-volumes/["Google Cloud용 NetApp Cloud Volumes Service"^]는 NFS 및 SMB를 통해 NAS 볼륨을 제공하는 클라우드 네이티브 파일 서비스로, 올플래시 성능을 제공합니다. 이 서비스를 통해 기존 애플리케이션을 포함한 모든 워크로드를 GCP 클라우드에서 실행할 수 있습니다. 일관된 고성능, 즉각적인 클론 생성, 데이터 보호 및 Google Compute Engine(GCE) 인스턴스에 대한 안전한 액세스를 제공하는 완전 관리형 서비스입니다.
